...
// ─── DOM REFS — declared first so every function below can use them ───────────
...
function getTimeContext(){
  ...
  return `Current time: ${timeStr} on ${dayName}, ${monthName} ${day}, ${year} (${timeOfDay}).` +
         (gapNote ? ' ' + gapNote : '');
}
...
function initScene(){
  ...
  rim.position.set(-2.5, 1.5, -2); scene.add(rim);
  const rim2 = new THREE.PointLight(0xb9a6ff, 2.6, 10);
  rim2.position.set(2, 0.5, -2.5); scene.add(rim2);
  ...
  controls.minDistance = 0.6; controls.maxDistance = 4;
...
let exprBlinkState = 'open';
let exprBlinkT     = 0;
const BLINK_MIN = 2.5, BLINK_MAX = 7.0;
let nextBlink = BLINK_MIN + Math.random() * (BLINK_MAX - BLINK_MIN);
...
function setVRMExpr(manager, names, weight){
  if(!manager) return;
  for(const n of names){ try{ manager.setValue(n, weight); }catch(e){} }
}
...
function tickExpressions(dt){
  ...
  if(exprBlinkState === 'open' && exprBlinkTimer >= nextBlink){
    ...
    nextBlink = BLINK_MIN + Math.random() * (BLINK_MAX - BLINK_MIN);
    exprBlinkTimer = 0;
  }
...
function tickProceduralIdle(dt){
  ...
  function setBoneRot(name, x, y, z){
  ...
  setBoneRot('hips', 0, 0, Math.sin(t * 1.1) * 0.008);

  const sd = breathe * 0.3;
  setBoneRot('leftUpperArm',  sd,  0.18, -1.25);
  setBoneRot('rightUpperArm', sd,  0.18,  1.25);
  setBoneRot('leftLowerArm',  0, 0,  0.05);
  setBoneRot('rightLowerArm', 0, 0, -0.05);

  setBoneRot('tail', Math.sin(t*0.8)*0.15*0.3, Math.sin(t*0.8)*0.15*0.5, Math.cos(t*0.6)*0.12);
  for(let i=1;i<=3;i++){
    const o=i*0.3;
    setBoneRot('tail'+i, Math.sin(t*0.8+o)*0.024, Math.sin(t*0.8+o)*0.048, Math.cos(t*0.6+o)*0.1);
  }
...
function animate(){
  ...
  if(useProceduralIdle){ tickProceduralIdle(dt); }
  else if(currentVrm){ currentVrm.update(dt); }
  tickExpressions(dt);
...
function onModelLoad(gltf){
  ...
  exprCurrent  = {}; exprTarget = 'neutral';
  ...
  if(_cachedClips.length){
    ...
    idleTime = 0; useProceduralIdle = true;
  }
...
function focusCameraOnHead(model, box){
  ...
  let targetY;
  if(headBone){
    const pos = new THREE.Vector3(); headBone.getWorldPosition(pos); targetY = pos.y;
  } else { targetY = box.min.y + (box.max.y - box.min.y) * 0.88; }
...
function tryPlayByPattern(pattern, loop){
  ...
  if(clip){ playClip(clip.name, loop); return true; } return false;
}
...
function reactToReply(text){
  setMood('speaking');
  setFacialExpression(exprFromReply(text), 4500);
...
async function regenerateLastReply(){
  ...
  fullChatScroll.querySelectorAll('.regen-bar').forEach(el=>el.remove());
  await dispatchReply(state.history);
}

async function continueLastReply(){
  if(state.isSending) return;
  const hist = [...state.history, { role:'user', content:'[continue your thought — keep going]' }];
  await dispatchReply(hist, true);
}

// dispatchReply — the single place that calls the API and updates the UI.
// historyForCall: the message array to send to OpenRouter
// skipStatePush: if true, don't push the assistant reply to state.history (for continue, we do want to push it though)
async function dispatchReply(historyForCall, skipStatePush=false){
  if(state.isSending) return;
  ...
  try{
    const reply = await callOpenRouter(historyForCall);
    // Always push to state.history so it's part of future context
    ...
    state.isSending=false; sendBtn.disabled=false;
  }
}
...
async function callOpenRouter(history){
  ...
  if(!content){
    throw new Error('Empty response from OpenRouter: '+(data?.error?.message||data?.detail||JSON.stringify(data||{})));
  }
...
if(!SpeechAPI){
  ...
  recognizer.addEventListener('result',e=>{
    ...
    for(let i=e.resultIndex;i<e.results.length;i++){
      const r=e.results[i];
      if(r.isFinal){ finalSegments[i]=r[0].transcript.trim(); interim=''; }
      else{ interim=r[0].transcript; }
    }
    const committed=finalSegments.filter(Boolean).join(' ');
    msgInput.value=[baseTextBeforeRecording,committed,interim].map(s=>s.trim()).filter(Boolean).join(' ');
    autoGrow();
  });
...
function startRecording(){
  ...
  baseTextBeforeRecording=msgInput.value.trim(); finalSegments=[];
  try{ recognizer.start(); }catch(e){ console.warn('Could not start recognizer',e); }
}
function stopRecording(){
  if(!recognizer) return;
  isRecording=false; micBtn.classList.remove('recording'); micStatus.textContent='';
...
function loadHistory(){
  try{
    const s=localStorage.getItem(HISTORY_KEY); if(!s) return [];
...
function renderHistory(){
  ...
  if(savedTs){ const ms=parseInt(savedTs,10); if(!isNaN(ms)) state.lastMessageTime=new Date(ms); }
...
function saveChatToFile(){
  if(!state.history.length){ pushSystemMsg('Nothing to save yet — start chatting first!'); return; }
  const payload={ version:3, savedAt:new Date().toISOString(), personaId:state.personaId, userName:state.userName, messages:state.history };
...
function loadChatFromFile(){
  ...
  input.addEventListener('change',async()=>{
    ...
    try{
      ...
      if(!Array.isArray(payload.messages)) throw new Error('Invalid chat file.');
      clearHistory();
      state.history=payload.messages;
      if(payload.personaId){ state.personaId=payload.personaId; applyPersonaTheme(); buildPersonaGrid(); }
      if(payload.userName&&!state.userName) state.userName=payload.userName;
    ...
    }catch(err){ pushSystemMsg('Could not load chat: '+(err.message||'invalid file')); }
  });
...
function persist(){
  localStorage.setItem(STORAGE_KEY,JSON.stringify({
    openRouterKey:state.openRouterKey, elevenLabsKey:state.elevenLabsKey,
    modelId:state.modelId, personaId:state.personaId,
    userName:state.userName, userAbout:state.userAbout,
...
document.getElementById('clear-chat-btn').addEventListener('click',()=>{
  clearHistory(); pushSystemMsg('Chat cleared — starting fresh.');
});

document.getElementById('clear-all-btn').addEventListener('click',()=>{
  state.openRouterKey=''; state.elevenLabsKey=''; state.userName='';
  state.userAbout=''; state.userNotes=''; state.personaId='sweet'; state.modelId=DEFAULT_MODEL_ID;
...
const _orLabel=document.querySelector('label[for="gemini-key"]');
if(_orLabel) _orLabel.textContent='OpenRouter API key';
const _orNote=orKeyInput?.closest('.field')?.querySelector('.note');
if(_orNote) _orNote.innerHTML=`Free at <a href="https://openrouter.ai/keys" target="_blank" rel="noopener">openrouter.ai/keys</a>. Default model: <code>${DEFAULT_MODEL_ID}</code>.`;
const _kcP=document.querySelector('#key-card p');
if(_kcP) _kcP.textContent="Before I can wake up properly I need an OpenRouter key to think with — it's free and takes like 30 seconds to grab.";
const _kcNote=document.querySelector('#key-card .note');
if(_kcNote) _kcNote.innerHTML='Get a free key at <a href="https://openrouter.ai/keys" target="_blank" rel="noopener">openrouter.ai/keys</a>.';
if(keyCardInput) keyCardInput.placeholder='paste your OpenRouter API key';
const _kcH2=document.querySelector('#key-card h2');
if(_kcH2) _kcH2.textContent="hii, I'm Luna!";

// Inject Save/Load buttons into settings sheet
const _sheetActions=document.querySelector('#settings-sheet .sheet-actions');
if(_sheetActions){
  const _saveBtn=document.createElement('button');
  _saveBtn.className='btn'; _saveBtn.textContent='Save chat'; _saveBtn.title='Download chat history as JSON';
  _saveBtn.addEventListener('click',()=>{ closeSettings(); saveChatToFile(); });
  const _loadBtn=document.createElement('button');
  _loadBtn.className='btn'; _loadBtn.textContent='Load chat'; _loadBtn.title='Import a previously saved chat JSON';
  _loadBtn.addEventListener('click',()=>{ closeSettings(); loadChatFromFile(); });
  _sheetActions.insertBefore(_saveBtn, _sheetActions.firstChild);
  _sheetActions.insertBefore(_loadBtn, _saveBtn.nextSibling);
}

// Inject CSS for regen bar
const _style=document.createElement('style');
_style.textContent=`
  .regen-bar{ display:flex; gap:6px; margin-top:6px; margin-left:2px; }
  .regen-action{ font-size:0.58rem; padding:5px 9px; gap:4px; opacity:0.65; transition:opacity 0.15s ease; }
...
function migrateOldStorage(){
  ...
  try{
    ...
    return { elevenLabsKey:p.elevenLabsKey||'', personaId:p.personaId||'sweet',
             userName:p.userName||'', userAbout:p.userAbout||'',
             userNotes:p.userNotes||'', modelUrl:p.modelUrl||'' };
  }catch(e){ return null; }
}
...
(function restore(){
  ...
  if(saved){
    try{
      ...
      state.userName=p.userName||''; state.userAbout=p.userAbout||''; state.userNotes=p.userNotes||'';
      ...
      orKeyInput.value=state.openRouterKey; elKeyInput.value=state.elevenLabsKey;
    ...
    if(migrated){
      ...
      state.personaId=migrated.personaId; state.userName=migrated.userName;
      state.userAbout=migrated.userAbout; state.userNotes=migrated.userNotes;
      state.modelUrl=migrated.modelUrl;
      userNameInput.value=state.userName; userAboutInput.value=state.userAbout;
      userNotesInput.value=state.userNotes; elKeyInput.value=state.elevenLabsKey;
      modelUrlInput.value=state.modelUrl;
      buildPersonaGrid();
    }else{
