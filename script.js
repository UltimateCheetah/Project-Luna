...
function createGamerRoom(){
  ...
  // ─── SHARED MATERIALS ───────────────────────────────────────────────────
  const wallMat = new THREE.MeshStandardMaterial({ color: 0x271a30, roughness: 0.5, metalness: 0.08 });
  const floorMat = new THREE.MeshStandardMaterial({ color: 0x150c18, roughness: 0.22, metalness: 0.6 });
  const pinkMat = new THREE.MeshStandardMaterial({ color: 0xff5ca0, roughness: 0.18, metalness: 0.25, emissive: 0xff2d7a, emissiveIntensity: 0.35 });
  const accentMat = new THREE.MeshStandardMaterial({ color: 0xff8fd4, roughness: 0.12, metalness: 0.3, emissive: 0xff5cb8, emissiveIntensity: 0.5 });
  const darkMat = new THREE.MeshStandardMaterial({ color: 0x0a0612, roughness: 0.55, metalness: 0.15 });
  const screenMat = new THREE.MeshStandardMaterial({ color: 0x18082e, roughness: 0.05, metalness: 0.05, emissive: 0xff8fd4, emissiveIntensity: 0.7 });
  const screenGlowMat = new THREE.MeshBasicMaterial({ color: 0xff8fd4, transparent: true, opacity: 0.2 });
  const deskMat = new THREE.MeshStandardMaterial({ color: 0x2e1c3c, roughness: 0.2, metalness: 0.55 });
  ...
  const creamMat = new THREE.MeshStandardMaterial({ color: 0xf5e6f0, roughness: 0.4, metalness: 0.05 });
  const whiteGlossMat = new THREE.MeshStandardMaterial({ color: 0xfaf5fa, roughness: 0.15, metalness: 0.1 });
  const glassMat = new THREE.MeshStandardMaterial({ color: 0xaac8e0, roughness: 0.05, metalness: 0.15, transparent: true, opacity: 0.3 });
  const skyOutsideMat = new THREE.MeshBasicMaterial({ color: 0x1a1040 });
  const woodMat = new THREE.MeshStandardMaterial({ color: 0xc4a0c8, roughness: 0.4, metalness: 0.05 });
  ...
  // ─── FLUFFY PINK CARPET (#ffb6c1) ──────────────────────────────────────
  const carpetGroup = new THREE.Group();
  const carpetMat = new THREE.MeshStandardMaterial({ color: 0xffb6c1, roughness: 0.95, metalness: 0.0 });
  const carpetBase = new THREE.Mesh(new THREE.CylinderGeometry(1.65, 1.72, 0.05, 48), carpetMat);
  carpetBase.position.set(0, 0.02, 0.5);
  carpetBase.receiveShadow = true;
  carpetGroup.add(carpetBase);

  const fluffMat = new THREE.MeshStandardMaterial({ color: 0xffb6c1, roughness: 0.95, metalness: 0.0, emissive: 0x1a1016, emissiveIntensity: 0.12 });
  for(let i = 0; i < 120; i++){
    const r = Math.sqrt(Math.random()) * 1.58;
    const angle = Math.random() * Math.PI * 2;
    const px = Math.cos(angle) * r;
    const pz = 0.5 + Math.sin(angle) * r;
    const h = 0.015 + Math.random() * 0.045;
    const fluff = new THREE.Mesh(new THREE.CylinderGeometry(0.018, 0.028, h, 6), fluffMat);
  ...
  for(let i = 0; i < 50; i++){
    ...
    const px = Math.cos(angle) * r, pz = 0.5 + Math.sin(angle) * r;
    const h = 0.02 + Math.random() * 0.05;
    const tuft = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.025, h, 5), fluffMat);
  ...
  // Carpet glow ring
  const carpetGlow = new THREE.Mesh(new THREE.RingGeometry(1.55, 1.8, 64),
  ...
  // Floor accent ring & grid (above carpet)
  const floorGlow = new THREE.Mesh(new THREE.RingGeometry(1.85, 1.9, 64),
  ...
  const synthWallPanel = new THREE.Mesh(new THREE.PlaneGeometry(10, 5.2),
  ...
  // ─── CEILING ────────────────────────────────────────────────────────────
  const ceiling = new THREE.Mesh(new THREE.PlaneGeometry(12, 9),
  ...
  function makeWindow(wx, wy, wz, rotY){
    ...
    const wFT = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.08, 0.08), creamMat); wFT.position.y = 0.8; wg.add(wFT);
    const wFB = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.08, 0.08), creamMat); wFB.position.y = -0.8; wg.add(wFB);
    const wFL = new THREE.Mesh(new THREE.BoxGeometry(0.08, 1.6, 0.08), creamMat); wFL.position.x = -0.56; wg.add(wFL);
    const wFR = new THREE.Mesh(new THREE.BoxGeometry(0.08, 1.6, 0.08), creamMat); wFR.position.x = 0.56; wg.add(wFR);
    const wGlass = new THREE.Mesh(new THREE.PlaneGeometry(1.04, 1.44), glassMat); wGlass.position.z = 0.01; wg.add(wGlass);
    const wSky = new THREE.Mesh(new THREE.PlaneGeometry(1.2, 1.6), skyOutsideMat); wSky.position.z = -0.04; wg.add(wSky);
    const wSill = new THREE.Mesh(new THREE.BoxGeometry(1.3, 0.04, 0.12), woodMat); wSill.position.y = -0.84; wSill.position.z = 0.05; wg.add(wSill);
    // Tiny city silhouette blocks
    const cb1 = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.4, 0.02), darkMat); cb1.position.set(-0.3, -0.4, -0.03); wg.add(cb1);
    const cb2 = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.3, 0.02), darkMat); cb2.position.set(0.1, -0.45, -0.03); wg.add(cb2);
    const cb3 = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.5, 0.02), darkMat); cb3.position.set(0.35, -0.35, -0.03); wg.add(cb3);
    const wGlow = new THREE.PointLight(0xaac8e8, 2, 3, 1.5); wGlow.position.z = 0.3; wg.add(wGlow);
    roomLights.push(wGlow);
    wg.position.set(wx, wy, wz); wg.rotation.y = rotY;
    roomGroup.add(wg);
  }
  ...
  const doorLeaf = new THREE.Mesh(new THREE.BoxGeometry(0.9, 2.6, 0.06), creamMat); doorLeaf.position.set(0.45, 1.3, 0); doorGroup.add(doorLeaf);
  const dp1 = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.6, 0.02), whiteGlossMat); dp1.position.set(0.45, 1.8, 0.04); doorGroup.add(dp1);
  const dp2 = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.6, 0.02), whiteGlossMat); dp2.position.set(0.45, 0.9, 0.04); doorGroup.add(dp2);
  const doorHandle = new THREE.Mesh(new THREE.SphereGeometry(0.04, 8, 8), accentMat); doorHandle.position.set(0.7, 1.3, 0.06); doorGroup.add(doorHandle);
  const dFT = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.08, 0.08), woodMat); dFT.position.set(0.45, 2.62, 0); doorGroup.add(dFT);
  const dFL = new THREE.Mesh(new THREE.BoxGeometry(0.06, 2.68, 0.08), woodMat); dFL.position.set(0, 1.34, 0); doorGroup.add(dFL);
  const dFR = new THREE.Mesh(new THREE.BoxGeometry(0.06, 2.68, 0.08), woodMat); dFR.position.set(0.9, 1.34, 0); doorGroup.add(dFR);
  doorGroup.position.set(5.97, 0, 2.8); doorGroup.rotation.y = -Math.PI / 2;
  ...
  function makeNeonLetter(letter, x, y){
    ...
    const seg = (sx, sy, sw, sh) => { const b = new THREE.Mesh(new THREE.BoxGeometry(sw, sh, 0.04), neonUwUMat); b.position.set(sx, sy, 0); g.add(b); };
    if(letter === 'U'){ seg(-0.2, 0.3, 0.05, 0.6); seg(0.2, 0.3, 0.05, 0.6); seg(0, 0, 0.3, 0.05); }
    else if(letter === 'w'){ seg(-0.2, 0.3, 0.05, 0.6); seg(0, 0.15, 0.05, 0.3); seg(0.2, 0.3, 0.05, 0.6); seg(-0.1, 0.1, 0.2, 0.05); seg(0.1, 0.1, 0.2, 0.05); }
    g.position.set(x, y, 0); return g;
  }
  uwuSignGroup.add(makeNeonLetter('U', -0.48, 0));
  uwuSignGroup.add(makeNeonLetter('w', 0, 0));
  uwuSignGroup.add(makeNeonLetter('U', 0.48, 0));
  const neonGlowPlane = new THREE.Mesh(new THREE.PlaneGeometry(2.0, 0.9),
    new THREE.MeshBasicMaterial({ color: 0xff69b4, transparent: true, opacity: 0.4, side: THREE.DoubleSide }));
  neonGlowPlane.position.z = -0.06; uwuSignGroup.add(neonGlowPlane);
  const neonGlowBig = new THREE.Mesh(new THREE.PlaneGeometry(2.6, 1.3),
    new THREE.MeshBasicMaterial({ color: 0xff69b4, transparent: true, opacity: 0.15, side: THREE.DoubleSide }));
  neonGlowBig.position.z = -0.08; uwuSignGroup.add(neonGlowBig);
  // Simple heart accent (two spheres + triangle cone)
  const heartL = new THREE.Mesh(new THREE.SphereGeometry(0.1, 8, 8), neonUwUMat); heartL.position.set(-0.08, -0.52, 0); uwuSignGroup.add(heartL);
  const heartR = new THREE.Mesh(new THREE.SphereGeometry(0.1, 8, 8), neonUwUMat); heartR.position.set(0.08, -0.52, 0); uwuSignGroup.add(heartR);
  const heartTip = new THREE.Mesh(new THREE.ConeGeometry(0.14, 0.2, 8), neonUwUMat); heartTip.position.set(0, -0.68, 0); heartTip.rotation.z = Math.PI; uwuSignGroup.add(heartTip);
  ...
  deskTop.position.y = 0.92; deskTop.castShadow = true; deskTop.receiveShadow = true;
  deskGroup.add(deskTop);
  const legPositions = [[-1.15, -0.45], [-0.38, -0.45], [0.38, -0.45], [1.15, -0.45], [-1.15, 0.45], [1.15, 0.45]];
  legPositions.forEach(([lx, lz]) => { const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.04, 0.92, 8), darkMat); leg.position.set(lx, 0.46, lz); leg.castShadow = true; deskGroup.add(leg); });
  const deskEdge = new THREE.Mesh(new THREE.BoxGeometry(2.6, 0.04, 0.05), ledPink);
  deskEdge.position.set(0, 0.87, 0.67); deskGroup.add(deskEdge);
  roomGroup.add(deskGroup);

  // ─── MONITOR ────────────────────────────────────────────────────────────
  const monitorGroup = new THREE.Group();
  monitorGroup.position.set(0, 1.75, -3.15);
  const screenPanel = new THREE.Mesh(new THREE.PlaneGeometry(1.5, 0.88), screenMat); monitorGroup.add(screenPanel);
  const glowPanel = new THREE.Mesh(new THREE.PlaneGeometry(1.58, 0.95), screenGlowMat); glowPanel.position.z = -0.015; monitorGroup.add(glowPanel);
  const bezelOuter = new THREE.Mesh(new THREE.BoxGeometry(1.7, 1.04, 0.05), darkMat); bezelOuter.position.z = -0.035; monitorGroup.add(bezelOuter);
  const monitorBack = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.95, 0.12), darkMat); monitorBack.position.z = -0.1; monitorGroup.add(monitorBack);
  const standNeck = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.06, 0.5, 16), deskMat); standNeck.position.set(0, -0.65, 0); monitorGroup.add(standNeck);
  const standBase = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.26, 0.05, 32), pinkMat); standBase.position.set(0, -0.92, 0); monitorGroup.add(standBase);
  const standRing = new THREE.Mesh(new THREE.TorusGeometry(0.24, 0.02, 8, 32), ledPink); standRing.rotation.x = Math.PI / 2; standRing.position.set(0, -0.89, 0); monitorGroup.add(standRing);
  roomGroup.add(monitorGroup);

  // Second portrait monitor
  const monitor2Group = new THREE.Group();
  monitor2Group.position.set(-1.1, 1.55, -3.08);
  const screen2 = new THREE.Mesh(new THREE.PlaneGeometry(0.55, 0.82), screenMat); monitor2Group.add(screen2);
  const bezel2 = new THREE.Mesh(new THREE.BoxGeometry(0.65, 0.92, 0.04), darkMat); bezel2.position.z = -0.03; monitor2Group.add(bezel2);
  const mon2Glow = new THREE.Mesh(new THREE.PlaneGeometry(0.62, 0.9), screenGlowMat); mon2Glow.position.z = -0.02; monitor2Group.add(mon2Glow);
  const stand2Neck = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.04, 0.3, 8), deskMat); stand2Neck.position.set(0, -0.44, 0); monitor2Group.add(stand2Neck);
  const stand2Base = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.12, 0.03, 16), pinkMat); stand2Base.position.set(0, -0.6, 0); monitor2Group.add(stand2Base);
  roomGroup.add(monitor2Group);

  // Small desk plant
  const deskPlantPot = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.04, 0.1, 8), creamMat); deskPlantPot.position.set(-0.55, 1.05, -3.0); roomGroup.add(deskPlantPot);
  const deskPlantLeaf = new THREE.Mesh(new THREE.SphereGeometry(0.07, 6, 5), new THREE.MeshStandardMaterial({ color: 0x7fd8a0, roughness: 0.5 })); deskPlantLeaf.position.set(-0.55, 1.14, -3.0); deskPlantLeaf.scale.set(1, 0.7, 1); roomGroup.add(deskPlantLeaf);

  // ─── CAT EARS ON MONITOR ────────────────────────────────────────────────
  function makeEar(x, lean){
    const g = new THREE.Group();
    const outer = new THREE.Mesh(new THREE.ConeGeometry(0.09, 0.22, 8), pinkMat); outer.position.y = 0.11; g.add(outer);
    const inner = new THREE.Mesh(new THREE.ConeGeometry(0.055, 0.14, 8), accentMat); inner.position.y = 0.13; g.add(inner);
    g.position.set(x, 2.24, -3.1); g.rotation.z = lean;
    return g;
  }
  ...
  kbGroup.position.set(0, 0.97, -2.55); kbGroup.rotation.x = -0.06;
  const kbBase = new THREE.Mesh(new THREE.BoxGeometry(0.85, 0.025, 0.32), pinkMat); kbGroup.add(kbBase);
  for(let r = 0; r < 5; r++){ const row = new THREE.Mesh(new THREE.BoxGeometry(0.78, 0.008, 0.04), accentMat); row.position.set(0, 0.02 + r * 0.01, -0.12 + r * 0.05); kbGroup.add(row); }
  roomGroup.add(kbGroup);

  // ─── MOUSE + MOUSEPAD ───────────────────────────────────────────────────
  const mousepad = new THREE.Mesh(new THREE.PlaneGeometry(0.35, 0.4), accentMat); mousepad.rotation.x = -Math.PI / 2; mousepad.position.set(0.6, 0.955, -2.45); roomGroup.add(mousepad);
  const mouseGeo = new THREE.SphereGeometry(0.05, 16, 8, 0, Math.PI*2, 0, Math.PI/2); const mouseMesh = new THREE.Mesh(mouseGeo, pinkMat); mouseMesh.scale.set(1, 0.55, 1.25); mouseMesh.position.set(0.6, 0.975, -2.5); roomGroup.add(mouseMesh);

  // ─── GAMING CHAIR ───────────────────────────────────────────────────────
  const chairGroup = new THREE.Group();
  chairGroup.position.set(0, 0.4, -1.6);
  const seat = new THREE.Mesh(new THREE.BoxGeometry(0.65, 0.1, 0.6), pinkMat); seat.position.y = 0.05; seat.castShadow = true; chairGroup.add(seat);
  const seatFront = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 0.65, 8), accentMat); seatFront.rotation.x = Math.PI / 2; seatFront.position.set(0, 0.02, 0.3); chairGroup.add(seatFront);
  const backrest = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.9, 0.09), accentMat); backrest.position.set(0, 0.5, -0.28); backrest.rotation.x = -0.08; backrest.castShadow = true; chairGroup.add(backrest);
  const wingL = new THREE.Mesh(new THREE.BoxGeometry(0.07, 0.6, 0.07), pinkMat); wingL.position.set(-0.31, 0.5, -0.26); wingL.rotation.z = 0.15; chairGroup.add(wingL);
  const wingR = new THREE.Mesh(new THREE.BoxGeometry(0.07, 0.6, 0.07), pinkMat); wingR.position.set(0.31, 0.5, -0.26); wingR.rotation.z = -0.15; chairGroup.add(wingR);
  const armL = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.05, 0.42), deskMat); armL.position.set(-0.36, 0.14, -0.05); chairGroup.add(armL);
  const armR = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.05, 0.42), deskMat); armR.position.set(0.36, 0.14, -0.05); chairGroup.add(armR);
  const lift = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.06, 0.28, 16), darkMat); lift.position.y = -0.2; chairGroup.add(lift);
  for(let i = 0; i < 5; i++){
    const a = (i/5) * Math.PI * 2;
    const spoke = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.035, 0.3), deskMat); spoke.position.set(Math.sin(a)*0.14, -0.34, Math.cos(a)*0.14); spoke.rotation.y = -a; chairGroup.add(spoke);
    const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.03, 8), darkMat); wheel.rotation.x = Math.PI/2; wheel.position.set(Math.sin(a)*0.28, -0.35, Math.cos(a)*0.28); chairGroup.add(wheel);
  }
  roomGroup.add(chairGroup);

  // ─── LEFT WALL: CATGIRL POSTER ─────────────────────────────────────────
  const leftPoster = new THREE.Mesh(new THREE.PlaneGeometry(2.0, 2.8), new THREE.MeshBasicMaterial({ map: catgirlTex }));
  leftPoster.position.set(-5.97, 2.8, -0.3); leftPoster.rotation.y = Math.PI / 2; roomGroup.add(leftPoster);
  const lFrame = new THREE.Mesh(new THREE.BoxGeometry(0.04, 2.9, 2.1), ledPink); lFrame.position.set(-5.99, 2.8, -0.3); lFrame.rotation.y = Math.PI / 2; roomGroup.add(lFrame);
  const leftPoster2 = new THREE.Mesh(new THREE.PlaneGeometry(1.6, 1.6), new THREE.MeshBasicMaterial({ map: nekoTex }));
  leftPoster2.position.set(-5.97, 1.1, 2.5); leftPoster2.rotation.y = Math.PI / 2; roomGroup.add(leftPoster2);
  const lFrame2 = new THREE.Mesh(new THREE.BoxGeometry(0.04, 1.7, 1.7), ledLilac); lFrame2.position.set(-5.99, 1.1, 2.5); lFrame2.rotation.y = Math.PI / 2; roomGroup.add(lFrame2);

  // ─── RIGHT WALL: CONTROLLER POSTER ─────────────────────────────────────
  const rightPoster = new THREE.Mesh(new THREE.PlaneGeometry(2.0, 2.0), new THREE.MeshBasicMaterial({ map: controllerTex }));
  rightPoster.position.set(5.97, 2.8, -0.2); rightPoster.rotation.y = -Math.PI / 2; roomGroup.add(rightPoster);
  const rFrame = new THREE.Mesh(new THREE.BoxGeometry(0.04, 2.1, 2.1), ledPink); rFrame.position.set(5.99, 2.8, -0.2); rFrame.rotation.y = -Math.PI / 2; roomGroup.add(rFrame);

  // ─── BACK WALL: PIXEL HEART POSTER ─────────────────────────────────────
  const heartPoster = new THREE.Mesh(new THREE.PlaneGeometry(0.9, 0.9), new THREE.MeshBasicMaterial({ map: heartTex }));
  heartPoster.position.set(2.8, 3.8, -3.96); roomGroup.add(heartPoster);
  const hFrameT = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.04, 0.02), ledPink); hFrameT.position.set(2.8, 4.28, -3.95); roomGroup.add(hFrameT);
  const hFrameB = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.04, 0.02), ledPink); hFrameB.position.set(2.8, 3.32, -3.95); roomGroup.add(hFrameB);

  // ─── BACK WALL: PAW PRINT POSTER ───────────────────────────────────────
  const pawPoster = new THREE.Mesh(new THREE.PlaneGeometry(0.9, 0.9), new THREE.MeshBasicMaterial({ map: pawTex }));
  pawPoster.position.set(-2.5, 3.8, -3.96); roomGroup.add(pawPoster);
  const pFrameT = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.04, 0.02), ledLilac); pFrameT.position.set(-2.5, 4.28, -3.95); roomGroup.add(pFrameT);
  const pFrameB = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.04, 0.02), ledLilac); pFrameB.position.set(-2.5, 3.32, -3.95); roomGroup.add(pFrameB);

  // ─── FLOATING SHELF (right) ────────────────────────────────────────────
  const shelf = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.05, 0.35), deskMat); shelf.position.set(3.5, 2.6, -2.3); shelf.castShadow = true; shelf.receiveShadow = true; roomGroup.add(shelf);
  const brackL = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.18, 0.03), pinkMat); brackL.position.set(2.65, 2.49, -2.3); roomGroup.add(brackL);
  const brackR = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.18, 0.03), pinkMat); brackR.position.set(4.35, 2.49, -2.3); roomGroup.add(brackR);
  const can = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 0.2, 16), pinkMat); can.position.set(3.15, 2.75, -2.25); roomGroup.add(can);
  const canTop = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 0.01, 16), accentMat); canTop.position.set(3.15, 2.85, -2.25); roomGroup.add(canTop);
  const miniPad = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.03, 0.26), accentMat); miniPad.position.set(3.9, 2.65, -2.35); miniPad.rotation.x = -0.35; roomGroup.add(miniPad);

  // ─── PLUSHIE ────────────────────────────────────────────────────────────
  const plushGroup = new THREE.Group();
  const plushBody = new THREE.Mesh(new THREE.SphereGeometry(0.1, 8, 8), pinkMat); plushBody.scale.set(1, 0.85, 0.8); plushGroup.add(plushBody);
  const plushHead = new THREE.Mesh(new THREE.SphereGeometry(0.08, 8, 8), accentMat); plushHead.position.set(0, 0.11, 0.05); plushGroup.add(plushHead);
  const plushEarL = new THREE.Mesh(new THREE.ConeGeometry(0.04, 0.07, 6), pinkMat); plushEarL.position.set(-0.05, 0.17, 0.03); plushEarL.rotation.z = 0.3; plushGroup.add(plushEarL);
  const plushEarR = new THREE.Mesh(new THREE.ConeGeometry(0.04, 0.07, 6), pinkMat); plushEarR.position.set(0.05, 0.17, 0.03); plushEarR.rotation.z = -0.3; plushGroup.add(plushEarR);
  const plushEyeL = new THREE.Mesh(new THREE.SphereGeometry(0.012, 4, 4), darkMat); plushEyeL.position.set(-0.025, 0.13, 0.1); plushGroup.add(plushEyeL);
  const plushEyeR = new THREE.Mesh(new THREE.SphereGeometry(0.012, 4, 4), darkMat); plushEyeR.position.set(0.025, 0.13, 0.1); plushGroup.add(plushEyeR);
  plushGroup.position.set(3.5, 2.67, -2.1); roomGroup.add(plushGroup);

  // ─── SECOND SHELF (left) ───────────────────────────────────────────────
  const shelf2 = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.04, 0.3), deskMat); shelf2.position.set(-3.5, 2.2, -2.5); shelf2.castShadow = true; shelf2.receiveShadow = true; roomGroup.add(shelf2);
  const brack2L = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.14, 0.03), pinkMat); brack2L.position.set(-4.15, 2.11, -2.5); roomGroup.add(brack2L);
  const brack2R = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.14, 0.03), pinkMat); brack2R.position.set(-2.85, 2.11, -2.5); roomGroup.add(brack2R);
  const plant2Pot = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.04, 0.1, 8), creamMat); plant2Pot.position.set(-3.2, 2.28, -2.45); roomGroup.add(plant2Pot);
  const plant2Leaf = new THREE.Mesh(new THREE.SphereGeometry(0.07, 6, 5), new THREE.MeshStandardMaterial({ color: 0x7fd8a0, roughness: 0.5 })); plant2Leaf.position.set(-3.2, 2.36, -2.45); plant2Leaf.scale.set(1, 0.7, 1); roomGroup.add(plant2Leaf);
  const miniPaw = new THREE.Mesh(new THREE.SphereGeometry(0.04, 6, 6), accentMat); miniPaw.scale.set(1, 0.5, 1); miniPaw.position.set(-3.8, 2.27, -2.4); roomGroup.add(miniPaw);

  // ─── LIGHTING ───────────────────────────────────────────────────────────
  roomLights = [];
  const ambient = new THREE.PointLight(0xff6ba8, 6, 12, 1.5); ambient.position.set(0, 3.5, -2); roomGroup.add(ambient); roomLights.push(ambient);
  const monGlow = new THREE.PointLight(0xff8fd4, 10, 5, 2); monGlow.position.set(0, 1.75, -3.3); roomGroup.add(monGlow); roomLights.push(monGlow);
  const mon2GlowLight = new THREE.PointLight(0xff8fd4, 4, 3, 2); mon2GlowLight.position.set(-1.1, 1.55, -3.0); roomGroup.add(mon2GlowLight); roomLights.push(mon2GlowLight);
  const deskGlow = new THREE.PointLight(0xff2d7a, 4, 2.5, 2); deskGlow.position.set(0, 0.4, -2.3); roomGroup.add(deskGlow); roomLights.push(deskGlow);
  const ceilLight = new THREE.PointLight(0xc9a6ff, 3, 7, 2); ceilLight.position.set(0, 5.5, 0.5); roomGroup.add(ceilLight); roomLights.push(ceilLight);
  const rightWash = new THREE.PointLight(0xff8fd4, 3, 4, 2); rightWash.position.set(5, 2.5, 1.5); roomGroup.add(rightWash); roomLights.push(rightWash);
  const leftWash = new THREE.PointLight(0xb9a6ff, 2.5, 4, 2); leftWash.position.set(-5, 2.5, -1); roomGroup.add(leftWash); roomLights.push(leftWash);
  const carpetGlowLight = new THREE.PointLight(0xffb6c1, 3, 3, 2); carpetGlowLight.position.set(0, 0.3, 0.5); roomGroup.add(carpetGlowLight); roomLights.push(carpetGlowLight);
